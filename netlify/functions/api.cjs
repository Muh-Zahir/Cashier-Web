  ...
  try {
    ...
    if (path === '/auth/login' && method === 'POST') {
      ...
      if (!username || !password) {
        return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'Username dan password wajib diisi' }) };
      }
      ...
      if (!user || !verifyPassword(password, user.password_hash)) {
        return { statusCode: 401, headers, body: JSON.stringify({ success: false, error: 'Username atau password salah' }) };
      }
    ...
    if (path === '/auth/logout' && method === 'POST') {
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: 'Logout berhasil' }) };
    }

    if (path === '/auth/me' && method === 'GET') {
      const currentUser = await getUserFromToken(token);
      if (!currentUser) {
        return { statusCode: 401, headers, body: JSON.stringify({ success: false, error: 'Sesi telah berakhir atau tidak valid' }) };
      }
      return { statusCode: 200, headers, body: JSON.stringify({ success: true, user: currentUser }) };
    }

    // ========== PRODUCTS ROUTES ==========
    if (path === '/products/meta/categories' && method === 'GET') {
      const res = await query('SELECT * FROM categories ORDER BY name ASC');
      return { statusCode: 200, headers, body: JSON.stringify({ success: true, data: res.rows }) };
    }

    if (path === '/products/meta/categories' && method === 'POST') {
      const { name, color } = body;
      if (!name) return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'Nama kategori wajib diisi' }) };
      ...
      return { statusCode: 201, headers, body: JSON.stringify({ success: true, data: cat.rows[0] }) };
    }
    ...
    if (path === '/products' && method === 'POST') {
      ...
      if (!name || price === undefined) {
        return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'Name and price are required' }) };
      }
      ...
      return { statusCode: 201, headers, body: JSON.stringify({ success: true, data: created.rows[0] }) };
    }
    ...
    if (productMatch) {
      ...
      if (subAction === 'toggle-status' && method === 'PATCH') {
        ...
        if (!prod.rows.length) return { statusCode: 404, headers, body: JSON.stringify({ success: false, error: 'Product not found' }) };
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, data: updated.rows[0] }) };
      }

      if (subAction === 'stock' && method === 'PATCH') {
        const { amount } = body;
        if (amount === undefined) return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'Amount is required' }) };
        const prod = await query('SELECT * FROM products WHERE id = ?', [id]);
        if (!prod.rows.length) return { statusCode: 404, headers, body: JSON.stringify({ success: false, error: 'Product not found' }) };
      ...
      if (method === 'GET') {
        const res = await query(`
          ...
          WHERE p.id = ?
        `, [id]);
        if (!res.rows.length) return { statusCode: 404, headers, body: JSON.stringify({ success: false, error: 'Product not found' }) };
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, data: res.rows[0] }) };
      }
      ...
      if (method === 'DELETE') {
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: 'Product deactivated' }) };
      }
    }
    ...
    if (path === '/transactions' && method === 'GET') {
      ...
      const res = await query(sql, queryArgs);
    ...
    if (path === '/transactions' && method === 'POST') {
      ...
      if (!items || !items.length) return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: 'Items required' }) };
      ...
      return {
        statusCode: 201,
        headers,
        body: JSON.stringify({ success: true, data: { ...finalTrx.rows[0], items: finalItems.rows } })
      };
    }
    ...
    if (trxMatch) {
      ...
      if (method === 'GET') {
        ...
        if (!trx.rows.length) return { statusCode: 404, headers, body: JSON.stringify({ success: false, error: 'Transaction not found' }) };
        const items = await query('SELECT * FROM transaction_items WHERE transaction_id = ?', [id]);
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, data: { ...trx.rows[0], items: items.rows } }) };
      }
      if (method === 'DELETE') {
        await query('DELETE FROM transaction_items WHERE transaction_id = ?', [id]);
        await query('DELETE FROM transactions WHERE id = ?', [id]);
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: 'Transaction deleted' }) };
      }
    }
    ...
    return { statusCode: 404, headers, body: JSON.stringify({ success: false, error: `Route ${path} not found` }) };
  } catch (err) {
